/**
 * EIEP14A feed parsing & normalisation (#225 split from eiep14a.ts).
 *
 * Owns the EIEP14A record → plan transformation seam: raw record shape,
 * retailer-name normalisation, tier extraction, field coercion, change-detection
 * hashing, and the JSON/CSV response parsers. Pure (no D1/KV I/O) so it can be
 * unit-tested without mocks. Consumed by eiep14a.ts (ingestion/orchestration).
 *
 * Public exports re-exported from eiep14a.ts so existing importers are unchanged.
 */

export interface EIEP14ARecord {
  PlanId?: string;
  plan_id?: string;
  id?: string;
  Retailer?: string;
...
export function parseJSONResponse(data: unknown): EIEP14ARecord[] {
...
export function parseCSVResponse(text: string): EIEP14ARecord[] {
