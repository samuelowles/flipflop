"""
Flick Electric bill parser.

Flick Electric (NZ, variable spot-price retailer) bills have a distinctive
layout:
- "Flick" branding near the top
- ICP number (15-digit) in the customer/supply header
- Billing period as a date range
- Usage breakdown showing total kWh
- Charges section with fixed daily charge and variable per-kWh rate
  (spot price passes through, often shown as the variable component)
- "Total due" or "Amount due" as the final amount
- Plan name (e.g. Off Peak, Flat, Freestyle)
...
class FlickParser(BaseParser):
    """Parser for Flick Electric residential electricity bills."""

    RETAILER_NAME = "Flick Electric"
    RETAILER_ID = "flick"
    ...
    def parse(self, file_path: str) -> ParserResult:
        ...
        # --- Meter type ---
        # Flick canonical bills always carry enough signal to classify the
...
# Register the parser
register_parser(FlickParser.RETAILER_ID, FlickParser)
