"""
Electric Kiwi bill parser.

Electric Kiwi (NZ) bills have a distinctive layout:
- "Electric Kiwi" branding near the top
...
- "Total due" as the final amount
- Plan name typically a fixed-rate product (Kiwi Fixed, Stay Ahead, On The Go)
...
class ElectricKiwiParser(BaseParser):
    """Parser for Electric Kiwi residential electricity bills."""

    RETAILER_NAME = "Electric Kiwi"
    RETAILER_ID = "electric_kiwi"
    ...
    def parse(self, file_path: str) -> ParserResult:
        ...
        # --- Usage kWh ---
        # Prefer a labelled "Total units/usage" line (Electric Kiwi day/night
        # bills list day & night components separately before a total); fall
        # back to the shared extractor.
    ...
    def _extract_electric_kiwi_usage(text: str) -> Optional[float]:
        ...
        patterns = [
            ...
            re.compile(r"(?:[Tt]otal|[Cc]onsumption)[\s:#-]*([\d,]+(?:\.\d+)?)\s*kWh", re.IGNORECASE),
        ]
    ...
    def _extract_electric_kiwi_total(text: str) -> Optional[int]:
        """Electric Kiwi bills use 'Total amount due' as the canonical total
        label. Falls back to the shared extract_dollars if not found.
        """
        patterns = [
            re.compile(r"[Tt]otal\s*(?:[Aa]mount\s*)?[Dd]ue[\s:#$-]*\$?\s*([\d,]+(?:\.\d{2})?)"),
...
# Register the parser
register_parser(ElectricKiwiParser.RETAILER_ID, ElectricKiwiParser)
